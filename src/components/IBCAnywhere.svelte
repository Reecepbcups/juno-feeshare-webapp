...
<script lang="ts">
	...
	// "{ \"forward\": { \"receiver\": \"JUNO_ADDR\", \"port\": \"transfer\", \"channel\": \"channel-###\" }}"
	let forward_chain_chain_id: string; // if we want to transfer with PFM, we get the channel_id from this
	let forwarded_chain_address: string;
	...
	const ibc_transfer = async () => {
		...
		// default memo
		let memo = `ibc.reece.sh | from ${chain.pretty_name} to ${to_chain.pretty_name}`

		if(forward_chain_chain_id.length > 0) {

			const pfm_chain = chains.find((chain) => chain.chain_id === forward_chain_chain_id);
			if (to_chain === undefined) {
				toast.error(`To chain ${to_chain_input} not found`, toast_style);
				throw new Error('Chain not found');
			}

			let pfm_chain_name = pfm_chain?.chain_name;
			if(!pfm_chain_name) {
				toast.error(`Forward chain ${forward_chain_chain_id} not found`, toast_style);
				throw new Error('Forward chain not found');
			}

			// use packet forward middleware memo
			let channel_id = get_channel(to_chain.chain_name, pfm_chain_name); // ex: channel-207 for CosmosHub -> Juno

			if(!channel_id) {
				toast.error(`No channel found for ${to_chain.chain_name} -> ${pfm_chain_name}`, toast_style);
...
<center>
	...
	<div id="denoms_to_send" style="display: none;" class="div_center">		
		...
		<!-- add a button which shows a packet forward middleware UI -->
		<!-- The user then enters anotehr chain they want to send the values too, and get their wallet address here -->
		<!-- then we add to the memo: "{ \"forward\": { \"receiver\": \"juno10r39fueph9fq7a6lgswu4zdsg8t3gxlq670lt0\", \"port\": \"transfer\", \"channel\": \"channel-207\" }}" -->

		<input type="submit" value="Packet Forward Middleware" on:click={() => {			
				// document.getElementById("packet-forward-middleware").style.display = "block";

				let element = document.getElementById("packet-forward-middleware");
				if(element) {
		...
		<div id="packet-forward-middleware" style="display: none;">
			...
			<!-- <input type="text" placeholder="forwarded wallet address" bind:value={forwarded_chain_address} /> -->
			
			<!-- call get wallet for chain -->
			<input type="submit" value="Get Wallet Address" on:click={() => 
				// forwarded_chain_address = get_wallet_for_chain(forward_chain_chain_id).then((res) => {
				// 	res.getAccounts().then((res) => {
				// 		forwarded_chain_address = res[0].address;
				// 	})
				// })

				get_wallet_for_chain(forward_chain_chain_id).then((res) => {
					res.getAccounts().then((res) => {
						forwarded_chain_address = res[0].address;
						console.log(forwarded_chain_address);
					})
				})							
				
			} />	
			
			<!-- show forwarded_chain_address as a p-->
			{#if forwarded_chain_address}
				<p>{forwarded_chain_address}</p>
			{/if}
		</div>
